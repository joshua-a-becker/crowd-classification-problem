...
calcWorstCase = function(distribution, truth) {
...
worstCaseByQuestion = ddply(d, .(task), summarize
      , err_med = median(response_1, na.rm=T)/unique(truth)
      , max_prop_wrong = calcWorstCase(response_1, unique(truth))
      )

ggplot(worstCaseByQuestion, aes(x=err_med, y=max_prop_wrong)) + geom_point()
